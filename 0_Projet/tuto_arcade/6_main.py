...
class MonJeu(arcade.Window):
    ...
    def __init__(self):
        ...
        # self.son_saut = pyglet.media.load('sounds/coin1.wav', streaming=False)
        # self.son_saut = pyglet.media.load('sounds/coin1.wav', streaming=False)
    ...
    def setup(self):
        ...
        self.sprite_joueur = arcade.Sprite(  # chemin image, echelle
            ":resources:images/animated_characters/female_adventurer/femaleAdventurer_idle.png",
            ECHELLE_PERSONNAGE
        )
        # positionner le personnage dans la fenêtre (pixel)
        self.sprite_joueur.center_x = 64
        self.sprite_joueur.center_y = 128
        # l'ajouter à sa SpriteList
        self.personnages.append(self.sprite_joueur)
        ...
        for coord in liste_coordonnees:
            ...
            self.tuiles.append(obstacle)

        # Placer des pièces
        for x in range(128, 1250, 256):
            piece = arcade.Sprite(":resources:images/items/coinGold.png", ECHELLE_PIECE)
            piece.center_x = x
            piece.center_y = 96
    ...
    def on_key_press(self, key, modifiers):
        ...
        if key == arcade.key.LEFT:
            self.sprite_joueur.change_x = -VITESSE_PERSONNAGE
    ...
    def on_update(self, delta_time):
        ...
        if self.sprite_joueur.right + MARGE_DROITE_VUE > self.xmin + LARGEUR_ECRAN:
            # right + marge == vd et vd == vg + L
            # vg == right + marge - L
            self.xmin = MARGE_DROITE_VUE + self.sprite_joueur.right - LARGEUR_ECRAN
            vue_change = True
        if self.sprite_joueur.bottom - MARGE_BASSE_VUE < self.ymin:
            # bot - marge == vb
            self.ymin = self.sprite_joueur.bottom - MARGE_BASSE_VUE
            vue_change = True
        if self.sprite_joueur.top + MARGE_HAUTE_VUE > self.ymin + HAUTEUR_ECRAN:
            self.ymin = self.sprite_joueur.top + MARGE_HAUTE_VUE - HAUTEUR_ECRAN
            vue_change = True
